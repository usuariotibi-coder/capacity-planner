# Planificador de Capacidad del Equipo - Guía de Despliegue

Guía completa para desplegar el backend de Team Capacity Planner en Railway.

## 📋 Requisitos Previos

- Cuenta de GitHub con el repositorio de código enviado
- Cuenta de Railway (gratis en [railway.app](https://railway.app))
- Conocimiento básico de variables de entorno y PostgreSQL

## 🚀 Despliegue Paso a Paso en Railway

### Paso 1: Crear Proyecto en Railway

1. Ve a [Railway.app](https://railway.app)
2. Haz clic en "Start a New Project"
3. Selecciona "Deploy from GitHub"
4. Conecta tu cuenta de GitHub
5. Selecciona el repositorio que contiene el código

### Paso 2: Agregar Base de Datos PostgreSQL

1. En tu proyecto de Railway, haz clic en "Add Service"
2. Selecciona "PostgreSQL"
3. Railway aprovisionará una base de datos PostgreSQL automáticamente

### Paso 3: Configurar Variables de Entorno

En el panel de Railway, ve a **Variables** y establece:

```
# Configuración de Django
DEBUG=False
SECRET_KEY=generate-a-secure-key-use-django-secret-key-generator
ALLOWED_HOSTS=your-app.railway.app,localhost
ENVIRONMENT=production

# Base de Datos (Railway proporciona esto automáticamente)
DATABASE_URL=postgresql://[automatically generated]

# Si usas variables de BD explícitas en lugar de DATABASE_URL:
DB_ENGINE=django.db.backends.postgresql
DB_NAME=capacity_planner
DB_USER=postgres
DB_PASSWORD=[auto-generated by Railway]
DB_HOST=[Railway-provided host]
DB_PORT=5432

# Configuración de CORS (Actualiza con tu URL del frontend)
CORS_ALLOWED_ORIGINS=https://your-frontend.railway.app,https://yourdomain.com

# Configuración de JWT (opcional, los valores por defecto son adecuados)
JWT_ACCESS_TOKEN_LIFETIME=86400
JWT_REFRESH_TOKEN_LIFETIME=604800
```

### Paso 4: Generar Clave Secreta

Genera una clave secreta segura de Django:

**Opción 1: Usando Python**
```bash
python -c "from django.core.management.utils import get_random_secret_key; print(get_random_secret_key())"
```

**Opción 2: Usando OpenSSL**
```bash
openssl rand -base64 32
```

Copia la clave generada y pégala en la variable `SECRET_KEY` de Railway.

### Paso 5: Configurar Parámetros de Construcción e Implementación

1. En Railway, ve a **Settings**
2. Establece estos valores:
   - **Build Command**: Dejar en blanco (usa Dockerfile)
   - **Start Command**: Dejar en blanco (usa Procfile)
   - **Port**: 8000
   - **Python Version**: 3.11

### Paso 6: Desplegar

1. Envía tu código a GitHub:
```bash
git add .
git commit -m "Ready for deployment"
git push origin main
```

2. Railway detectará automáticamente los cambios y volverá a desplegar

### Paso 7: Ejecutar Migraciones Iniciales

Después del primer despliegue:

1. Ve al panel de Railway
2. Abre el servicio web
3. Haz clic en la pestaña "Logs"
4. Busca los registros de despliegue - las migraciones deberían ejecutarse automáticamente a través del Procfile

Si las migraciones no se ejecutaron:
1. Haz clic en "Settings" en el servicio web
2. Busca la sección "Execute Command"
3. Ejecuta: `python manage.py migrate`

### Paso 8: Crear Superusuario

Accede a la terminal de tu aplicación desplegada:

```bash
# A través de Railway CLI
railway run python manage.py createsuperuser

# O a través del panel de Railway:
# 1. Selecciona tu servicio web
# 2. Ve a la pestaña "Deployment"
# 3. Haz clic en el despliegue en ejecución
# 4. Abre shell/terminal
# 5. Ejecuta: python manage.py createsuperuser
```

## 🔒 Lista de Verificación de Variables de Entorno

- [ ] `DEBUG=False` (nunca True en producción)
- [ ] `SECRET_KEY` establecida en una cadena aleatoria segura
- [ ] `ALLOWED_HOSTS` incluye tu dominio de Railway
- [ ] `DATABASE_URL` configurada para PostgreSQL
- [ ] `CORS_ALLOWED_ORIGINS` incluye tu URL del frontend
- [ ] Todas las variables de base de datos establecidas (si no usas DATABASE_URL)

## 🧪 Prueba del Despliegue

Después de que se complete el despliegue:

```bash
# Prueba la disponibilidad de la API
curl https://your-app.railway.app/api/

# Prueba el endpoint de autenticación
curl -X POST https://your-app.railway.app/api/token/ \
  -H "Content-Type: application/json" \
  -d '{"username": "admin", "password": "password"}'

# Prueba con token
curl -H "Authorization: Bearer <your-token>" \
  https://your-app.railway.app/api/employees/
```

## 📊 Monitoreo y Registros

### Ver Registros
1. Ve a tu proyecto de Railway
2. Selecciona el servicio web
3. Haz clic en la pestaña "Logs"
4. Ve los registros de la aplicación en tiempo real

### Problemas Comunes

#### Error: "relation does not exist"
- Las migraciones no se ejecutaron
- Solución: Ejecuta manualmente `python manage.py migrate` en el shell de Railway

#### Error: "CORS policy: blocked"
- La URL del frontend no está en CORS_ALLOWED_ORIGINS
- Solución: Actualiza CORS_ALLOWED_ORIGINS en las variables de Railway

#### Error: "SECRET_KEY not set"
- La variable de entorno SECRET_KEY está faltando
- Solución: Agrégala a las variables de Railway

#### Error: "could not connect to database"
- Las credenciales de la base de datos son incorrectas o la base de datos no está funcionando
- Solución: Verifica que las variables de BD coincidan con los valores proporcionados por Railway

## 🔄 Despliegue Continuo

Railway vuelve a desplegar automáticamente cuando envías cambios a tu rama principal:

1. Realiza cambios localmente
2. Confirma y envía:
   ```bash
   git add .
   git commit -m "Tu mensaje"
   git push origin main
   ```
3. Railway detecta el envío y vuelve a desplegar automáticamente
4. Revisa los registros para confirmar el despliegue exitoso

## 📈 Escalabilidad para 50+ Usuarios

Railway maneja automáticamente la escalabilidad. Configuración:

1. **Memoria**: El valor por defecto de 256MB es suficiente para 50 usuarios
   - Para 100+ usuarios, aumenta a 512MB

2. **CPU**: CPU compartida suficiente para 50 usuarios
   - Para 100+ usuarios, actualiza a dedicada

3. **Base de Datos**: PostgreSQL por defecto es suficiente
   - Para alto volumen, considera actualizar el plan

### Monitorear Rendimiento

1. Ve al panel de Railway
2. Visualiza la pestaña "Metrics"
3. Monitorea el uso de CPU, memoria y red

## 🔐 Mejores Prácticas de Seguridad

- [ ] Usar una SECRET_KEY fuerte (mínimo 32 caracteres)
- [ ] Establecer DEBUG=False en producción
- [ ] Usar solo HTTPS (Railway proporciona SSL automáticamente)
- [ ] Rotar regularmente secretos y tokens
- [ ] Habilitar las características de seguridad de Railway
- [ ] Configurar monitoreo y alertas
- [ ] Actualizar regularmente las dependencias

## 🚨 Copia de Seguridad y Recuperación

### Copias de Seguridad de la Base de Datos

Railway realiza automáticamente copias de seguridad de las bases de datos PostgreSQL.

Para descargar una copia de seguridad:
1. Ve al servicio PostgreSQL en Railway
2. Haz clic en la pestaña "Data"
3. Descarga el archivo de copia de seguridad

### Copia de Seguridad Manual

```bash
# A través de Railway CLI
railway run pg_dump $DATABASE_URL > backup.sql
```

## 🎯 Lista de Verificación de Producción

Antes de ir en vivo:

- [ ] SECRET_KEY es segura y está establecida
- [ ] DEBUG es False
- [ ] ALLOWED_HOSTS incluye tu dominio
- [ ] La base de datos es PostgreSQL
- [ ] CORS_ALLOWED_ORIGINS está configurada para el frontend
- [ ] Todas las migraciones aplicadas
- [ ] Superusuario creado
- [ ] Archivos estáticos configurados
- [ ] Configuración de correo electrónico (si es necesario)
- [ ] Certificado SSL (automático en Railway)
- [ ] Monitoreo habilitado
- [ ] Copias de seguridad configuradas
- [ ] El equipo tiene acceso al panel de Railway

## 📞 Soporte y Solución de Problemas

### Soporte de Railway
- [Documentación de Railway](https://docs.railway.app)
- [Discord Comunitario de Railway](https://discord.gg/railway)

### Soporte de Django
- [Documentación de Django](https://docs.djangoproject.com)
- [Django REST Framework](https://www.django-rest-framework.org)

### Comandos Comunes

```bash
# Conectarse a Railway CLI
railway login
railway link  # Vincular proyecto local a proyecto de Railway

# Ejecutar comandos remotamente
railway run python manage.py shell
railway run python manage.py migrate
railway run python manage.py createsuperuser

# Ver registros
railway logs

# Volver a desplegar
railway deploy
```

## 🎓 Recursos Adicionales

- [Tutorial de Django en Railway](https://docs.railway.app/guides/django)
- [Lista de Verificación de Despliegue de Django](https://docs.djangoproject.com/en/4.2/howto/deployment/checklist/)
- [Mejores Prácticas de PostgreSQL](https://www.postgresql.org/docs/current/sql-syntax.html)

---

**Última Actualización**: 6 de enero de 2026
**Versión**: 1.0.0

Para obtener ayuda adicional, consulta API_DOCUMENTATION.md o README.md
