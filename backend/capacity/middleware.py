    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        if auth_header.startswith('Bearer '):
            ...
            try:
                ...
                # Always cleanup stale sessions first, but exclude the current session.
                stale_query = UserSession.objects.filter(
                    is_active=True,
                    last_activity__lt=inactive_threshold,
                )
                if session_id:
                    stale_query = stale_query.exclude(id=session_id)
                stale_query.update(is_active=False)
