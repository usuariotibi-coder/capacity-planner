...
from .models import (
    ...
    DepartmentWeeklyTotal, UserDepartment, OtherDepartment,
...
from .finance_import import run_finance_import
...
class ProjectViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        include_closed = _query_param_as_bool(
            self.request.query_params.get('include_closed'),
            default=False,
        )
    ...
    def statistics(self, request, pk=None):
        ...
        # Group by effective department (department_override, else employee's own)
        dept_stats = {}
        for assignment in assignments:
            dept = assignment.department_override or assignment.employee.department
    ...
    def update_budget_hours(self, request, pk=None):
        ...
        if department and not _can_edit_department(request.user, department):
            raise PermissionDenied("No permission to modify this department's budget.")
...
class AssignmentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Optional filter: effective department (used by frontend to reduce payload).
        # Matches either an explicit department_override, or the employee's own
        # department when no override is set (default/backward-compatible case).
    ...
    def _ensure_assignment_edit_permission(self, employee, department_override=None):
        """
        Permission is checked against the EFFECTIVE department (department_override
        when present, otherwise the employee's own department) — i.e. whoever owns
        that department's row is the one authorized to edit it, whether the hours
        belong to their own staff or to someone supporting them from another team.
        """
        if _has_full_access(self.request.user):
            return
        if _is_read_only_user(self.request.user):
    ...
    def perform_create(self, serializer):
        ...
        department_override = (
            serializer.validated_data.get('department_override')
            or self.request.data.get('department_override')
            or None
        )
    ...
    def perform_update(self, serializer):
        ...
        department_override = serializer.validated_data.get(
    ...
    def perform_destroy(self, instance):
        ...
        self._ensure_week_is_editable(instance.week_start_date)
        instance.delete()
    ...
    def summary_by_project_dept(self, request):
        ...
        # Group by EFFECTIVE department: department_override when set (an employee
        # supporting another department), otherwise the employee's own department.
        qs = qs.annotate(
            effective_department=Coalesce(
                'department_override', 'employee__department', output_field=CharField()
            )
        )

        if department:
            qs = qs.filter(effective_department=department)

        rows = (
            qs.values('project_id', 'effective_department')
            .annotate(
                utilized=Sum(
                    Case(
        ...
        totals = {}
        for row in rows:
            dept = row.get('effective_department')
            if not dept:
                continue
            key = (str(row['project_id']), dept)
            totals[key] = {
                'utilized': float(row['utilized'] or 0),
                'forecast': float(row['forecast'] or 0),
            }

        # Past weeks are additionally sourced from Finance actuals (real hours by
        # project/department/week, with no per-employee breakdown). These are always
        # "utilized" since actuals only ever cover closed/past weeks.
        actuals_qs = ProjectDepartmentWeeklyActual.objects.filter(
            project_id__in=project_ids,
            week_start_date__lt=split_date,
        )
        if department:
            actuals_qs = actuals_qs.filter(department=department)
        actuals_rows = (
            actuals_qs.values('project_id', 'department')
            .annotate(actual_hours=Sum('hours'))
        )
        for row in actuals_rows:
            key = (str(row['project_id']), row['department'])
            entry = totals.setdefault(key, {'utilized': 0.0, 'forecast': 0.0})
            entry['utilized'] += float(row['actual_hours'] or 0)
        ...
        results = [
            ...
            for (project_id, dept), value in totals.items()
        ]
    ...
    def by_week(self, request):
        ...
        for assignment in assignments:
            ...
            # Track by effective department (department_override, else employee's own)
            dept = assignment.department_override or assignment.employee.department
    ...
    def capacity_by_dept(self, request):
        ...
        # Add allocated hours from assignments (effective department: department_override
        # when the employee is supporting another department, else their own)
        for assignment in assignments:
            dept = assignment.department_override or assignment.employee.department
            if dept not in dept_stats:
                dept_stats[dept] = {
        ...
        # Calculate allocations by effective department
        dept_allocated = {}
        for assignment in assignments:
            dept = assignment.department_override or assignment.employee.department
    ...
    def utilization_report(self, request):
        ...
        for assignment in assignments:
            ...
            # 'allocated' intentionally sums ALL of this employee's hours regardless of
            # department, so over/under-utilization reflects their real total workload
            # even when some of it is spent supporting another department.
            employee_util[emp_id]['allocated'] += assignment.hours
            assignment_department = assignment.department_override or assignment.employee.department
            employee_util[emp_id]['assignments'].append({
                'project': assignment.project.name,
                'hours': assignment.hours,
                'stage': assignment.stage,
                'department': assignment_department,
                'is_support': assignment_department != assignment.employee.department,
            })
...
class DailyTimeEntryViewSet(viewsets.ModelViewSet):
    """
    ViewSet for DailyTimeEntry: daily actuals log per employee (project work,
    OIL/issue-resolution, indirect time, or vacation).

    Endpoints:
        GET    /api/daily-time-entries/                 - List (filterable)
        POST   /api/daily-time-entries/                 - Create
        GET    /api/daily-time-entries/{id}/             - Retrieve
        PUT/PATCH /api/daily-time-entries/{id}/          - Update
        DELETE /api/daily-time-entries/{id}/             - Delete
        GET    /api/daily-time-entries/by-project/       - OIL + PROJECT hours logged per project (reporting only)

    Query Parameters:
        - employee: filter by employee UUID
        - department: filter by employee's department
        - start_date / end_date: filter by date range (YYYY-MM-DD, inclusive)
        - entry_type: filter by PROJECT | OIL | IND | VAC

    Permissions mirror Assignment: each department can only log/edit time for
    their own staff (BUILD/MFG share edit rights, same as everywhere else);
    full-access users (PM, BI, staff/superuser) can edit any department.
    """
    queryset = DailyTimeEntry.objects.all().select_related('employee', 'project')
    serializer_class = DailyTimeEntrySerializer
    permission_classes = [IsAuthenticated]
    pagination_class = LargeResultsSetPagination
    filter_backends = [DjangoFilterBackend, filters.OrderingFilter]
    filterset_fields = ['employee', 'project', 'entry_type', 'date']
    ...
    def _ensure_edit_permission(self, employee):
        if _has_full_access(self.request.user):
            return
        if _is_read_only_user(self.request.user):
            raise PermissionDenied("Read-only access.")
        if not employee or not _can_edit_department(self.request.user, employee.department):
            raise PermissionDenied("No permission to log time for this department.")

    def perform_create(self, serializer):
        employee = serializer.validated_data.get('employee')
        self._ensure_edit_permission(employee)
        try:
            serializer.save()
        except IntegrityError:
            raise ValidationError(
                'Ya existe una entrada de este tipo para este empleado, fecha y proyecto.'
            )
    ...
    def perform_destroy(self, instance):
        self._ensure_edit_permission(instance.employee)
        instance.delete()

    @action(detail=False, methods=['get'], url_path='by-project')
    def by_project(self, request):
        """
        OIL + PROJECT hours logged per project, for a date range. Informational only —
        does NOT modify ProjectBudget.hours_utilized (which PMs maintain manually);
        this is here so PMs can see logged actuals and factor them in themselves.
        """
        project_ids_param = (request.query_params.get('project_ids') or '').strip()
        qs = self.get_queryset().filter(entry_type__in=[TimeEntryType.PROJECT, TimeEntryType.OIL])
        ...
        rows = (
            qs.values('project_id', 'entry_type')
        ...
        for row in rows:
            ...
            entry = results.setdefault(pid, {'project_id': pid, 'project_hours': 0.0, 'oil_hours': 0.0})
            if row['entry_type'] == TimeEntryType.PROJECT:
                entry['project_hours'] = float(row['hours'] or 0)
            else:
                entry['oil_hours'] = float(row['hours'] or 0)
...
class ActivityLogViewSet(viewsets.ModelViewSet):
    ...
    Provides access to activity logs for audit trail:
    - Filtering by user and model
    - Ordering by timestamp
    - Pagination with large result set
    - Client-created logs are validated server-side for action/model integrity

    Endpoints:
        GET /api/activity-logs/ - List all activity logs
        GET /api/activity-logs/{id}/ - Get log entry details
        POST /api/activity-logs/ - Create a new activity log (validated server-side)
    ...
    Query Parameters:
        ...
        - page_size: Items per page
    """
    ALLOWED_ACTIONS = {'created', 'updated', 'deleted', 'viewed'}
    ALLOWED_MODELS = {'Employee', 'Project', 'Assignment', 'ProjectBudget',
                      'ProjectChangeOrder', 'DepartmentStageConfig',
    ...
    def perform_create(self, serializer):
        """Validate and set user when creating an activity log."""
        action = serializer.validated_data.get('action', '').strip().lower()
        model_name = serializer.validated_data.get('model_name', '').strip()

        if action not in self.ALLOWED_ACTIONS:
            raise ValidationError({'action': f'Invalid action. Must be one of: {sorted(self.ALLOWED_ACTIONS)}'})
        if model_name not in self.ALLOWED_MODELS:
            raise ValidationError({'model_name': f'Invalid model. Must be one of: {sorted(self.ALLOWED_MODELS)}'})
...
class ProjectDepartmentWeeklyActualViewSet(viewsets.ReadOnlyModelViewSet):
    """
    Read-only access to Finance actuals (real historical hours per project/
    department/week). Written exclusively by the Finance import endpoint
    (see FinanceImportViewSet.import_file), never through this ViewSet.
    """
    queryset = ProjectDepartmentWeeklyActual.objects.all().select_related('project')
    serializer_class = ProjectDepartmentWeeklyActualSerializer
    permission_classes = [IsAuthenticated]
    pagination_class = LargeResultsSetPagination
    filter_backends = [DjangoFilterBackend, filters.OrderingFilter]
    filterset_fields = ['department', 'week_start_date', 'project']
    ordering_fields = ['week_start_date', 'department']
...
class FinanceImportViewSet(viewsets.ReadOnlyModelViewSet):
    """
    Upload endpoint for the recurring Finance actuals report, plus a read-only
    audit trail of past uploads.

    POST /api/finance-imports/import_file/ (multipart, field name 'file')
        Full-access users only. Parses the uploaded .xlsx, computes week-over-week
        diffs against what's already on file, writes ProjectDepartmentWeeklyActual
        rows for any newly-covered weeks, and returns a summary.
    GET /api/finance-imports/ - list past import runs
    """
    queryset = FinanceImportLog.objects.all()
    serializer_class = FinanceImportLogSerializer
    permission_classes = [IsAuthenticated]
    pagination_class = StandardResultsSetPagination

    @action(detail=False, methods=['post'], url_path='import-file')
    def import_file(self, request):
        if not _has_full_access(request.user):
            raise PermissionDenied("Only administrators can import Finance actuals.")

        uploaded_file = request.FILES.get('file')
        if not uploaded_file:
            return Response({'detail': "No file uploaded (expected multipart field 'file')."}, status=status.HTTP_400_BAD_REQUEST)

        try:
            log = run_finance_import(uploaded_file, uploaded_by=request.user, file_name=uploaded_file.name)
        except ValueError as exc:
            return Response({'detail': str(exc)}, status=status.HTTP_400_BAD_REQUEST)
        except Exception as exc:
            logging.getLogger(__name__).exception("Finance import failed")
            return Response({'detail': f'Import failed: {exc}'}, status=status.HTTP_400_BAD_REQUEST)

        return Response(FinanceImportLogSerializer(log).data, status=status.HTTP_201_CREATED)
...
# ==================== USER REGISTRATION VIEWS (removed) ====================
# Self-service registration (POST /api/register/, email/code verification) has
# been removed: it let anyone with a domain-matching email grant themselves any
# department -- including OTHER/BUSINESS_INTELLIGENCE, which is full admin
# access -- with no human approval step. Account creation now goes exclusively
# through RegisteredUserViewSet (BI-only, see below), which creates users
# active immediately with an explicitly assigned department.
#
