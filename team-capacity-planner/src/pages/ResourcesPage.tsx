...
import { getAllWeeksWithNextYear, normalizeWeekStartDate, parseISODate, formatToISO, getWeekStart, getWeekNumber } from '../utils/dateUtils';
import { WeekNumberDatePicker } from '../components/WeekNumberDatePicker';
import { Plus, Trash2, Edit2, ChevronDown, ChevronUp, Calendar, X, Download, FileSpreadsheet, Search } from 'lucide-react';
...
export function ResourcesPage() {
  ...
  // Availability filters (merged in from the former standalone Availability page)
  const [selectedWeek, setSelectedWeek] = useState<string>(() => formatToISO(getWeekStart(new Date())));
  ...
  // --- Availability helpers (merged in from the former standalone Availability page) ---

  const weekNumber = (() => {
    try {
      return getWeekNumber(selectedWeek);
    } catch {
      return null;
    }
  })();

  // Whether the employee was on staff during the given week, based on hire/termination
  // dates. A week that only partially overlaps hire/termination still counts as active.
  const isEmployeeActiveForWeek = (employee: Employee, weekStartISO: string): boolean => {
    const weekStart = parseISODate(weekStartISO);
    const weekEnd = new Date(weekStart);
    weekEnd.setDate(weekEnd.getDate() + 6);
    if (employee.hireDate) {
      const hire = parseISODate(employee.hireDate);
      if (weekEnd < hire) return false;
    }
    if (employee.terminationDate) {
      const termination = parseISODate(employee.terminationDate);
      if (weekStart > termination) return false;
    }
    return true;
  };

  // Whether the employee should be treated as active *today*, independent of the stored
  // is_active flag (which is only refreshed on save). A termination date in the past
  // always wins.
  const isEffectivelyActive = (employee: Employee): boolean => {
    if (!employee.isActive) return false;
    if (employee.terminationDate) {
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      const termination = parseISODate(employee.terminationDate);
      if (termination <= today) return false;
    }
    return true;
  };

  const getWeekBreakdown = (employee: Employee, weekStartISO: string) => {
    const normalizedWeek = normalizeWeekStartDate(weekStartISO);
    const weekAssignments = assignments.filter(
      (a) => a.employeeId === employee.id && normalizeWeekStartDate(a.weekStartDate) === normalizedWeek && hasWorkHours(a)
    );

    const breakdown = new Map<string, { department: Department; projectId?: string; hours: number; isSupport: boolean }>();
    weekAssignments.forEach((a) => {
      const effectiveDept = (a.department || employee.department) as Department;
      const hours = getAssignmentHours(a);
      if (hours <= 0) return;
      const key = `${effectiveDept}|${a.projectId || ''}`;
      ...
      if (existing) {
        ...
        breakdown.set(key, {
          ...
          hours,
          isSupport: effectiveDept !== employee.department,
        });
      }
    });

    const assignedHours = weekAssignments.reduce((sum, a) => sum + getAssignmentHours(a), 0);
    const roundedAssigned = Math.round(assignedHours * 100) / 100;
    const activeThisWeek = isEmployeeActiveForWeek(employee, weekStartISO);
    const capacityThisWeek = activeThisWeek ? getEmployeeCapacity(employee) : 0;

    return {
      activeThisWeek,
      assignedHours: roundedAssigned,
      capacityHours: capacityThisWeek,
      availableHours: Math.round((capacityThisWeek - roundedAssigned) * 100) / 100,
      breakdown: Array.from(breakdown.values()).sort((a, b) => b.hours - a.hours),
    };
  };
  ...
  const isCompanyPlaceholder = (e: Employee) =>
    e.isSubcontractedMaterial &&
    e.subcontractCompany &&
    e.subcontractCompany === e.name &&
    (e.capacity === 0 || e.capacity === 0.0);

  const matchesAvailabilityFilters = (employee: Employee): boolean => {
    if (search.trim() && !employee.name.toLowerCase().includes(search.trim().toLowerCase())) {
      return false;
    }
    if (onlyAvailable) {
      const { availableHours } = getWeekBreakdown(employee, selectedWeek);
  ...
  const weekTotals = useMemo(() => {
    return employees
      .filter((e) => !isPlaceholderEmployee(e) && !isCompanyPlaceholder(e))
      .filter(matchesAvailabilityFilters)
      .reduce(
        (acc, employee) => {
          const { capacityHours, assignedHours, availableHours } = getWeekBreakdown(employee, selectedWeek);
          return {
            capacity: acc.capacity + capacityHours,
            assigned: acc.assigned + assignedHours,
            available: acc.available + availableHours,
          };
        },
        { capacity: 0, assigned: 0, available: 0 }
      );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [employees, assignments, selectedWeek, search, onlyAvailable]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setFormData({ name: '', role: '', department: getDefaultDepartment(), capacity: DEFAULT_WEEKLY_CAPACITY, isSubcontractedMaterial: false, subcontractCompany: '', hireDate: '', terminationDate: '' });
    setIsFormOpen(false);
  };

  const handleEdit = (employee: Employee) => {
    if (!canEditEmployee(employee.department)) return;
    setFormData({ ...employee, capacity: getEmployeeCapacity(employee), hireDate: employee.hireDate || '', terminationDate: employee.terminationDate || '' });
  ...
  const getDepartmentColor = (dept: Department) => {
    const colors: Record<Department, string> = {
      ...
      'PURCHASING': 'bg-[#effafa] border-[#c8e6e6]',
    };
    return colors[dept];
  };
  ...
  return (
    <div className="brand-page-shell resources-page flex flex-col h-full min-h-0 overflow-hidden">
      ...
      {/* Availability filter bar - merged in from the former standalone Availability page */}
      <div className="px-4 sm:px-8 pt-4">
        <div className="bg-white border border-[#e5e0eb] rounded-lg p-3 flex flex-col md:flex-row gap-2 md:items-center">
          <div className="flex items-center gap-2">
            <label className="text-xs font-semibold text-[#6c6480] whitespace-nowrap">
              {language === 'es' ? 'Semana' : 'Week'}{weekNumber ? ` (CW${weekNumber})` : ''}
            </label>
            <WeekNumberDatePicker value={selectedWeek} onChange={setSelectedWeek} language={language} compact />
          </div>
          ...
          <div className="relative flex-1 min-w-[160px]">
            ...
            <input
              ...
              className="w-full text-sm border border-gray-300 rounded-md pl-8 pr-2.5 py-1.5"
            />
          </div>

          <label className="flex items-center gap-1.5 text-xs font-medium text-[#6c6480] whitespace-nowrap cursor-pointer">
            <input type="checkbox" checked={onlyAvailable} onChange={(e) => setOnlyAvailable(e.target.checked)} />
            {language === 'es' ? 'Solo con horas disponibles' : 'Only with hours available'}
          </label>
        </div>

        {/* Week summary */}
        <div className="grid grid-cols-3 gap-2 mt-2">
          <div className="bg-white border border-[#e5e0eb] rounded-lg p-3">
            <div className="text-[10px] uppercase tracking-wide text-gray-500">{language === 'es' ? 'Capacidad total' : 'Total capacity'}</div>
            <div className="text-lg font-bold text-[#2e1a47]">{formatHours(weekTotals.capacity)}h</div>
          </div>
          <div className="bg-white border border-[#e5e0eb] rounded-lg p-3">
            <div className="text-[10px] uppercase tracking-wide text-gray-500">{language === 'es' ? 'Horas asignadas' : 'Assigned hours'}</div>
            <div className="text-lg font-bold text-[#2e1a47]">{formatHours(weekTotals.assigned)}h</div>
          </div>
          <div className={`rounded-lg p-3 border ${weekTotals.available < 0 ? 'bg-red-50 border-red-200' : 'bg-emerald-50 border-emerald-200'}`}>
            <div className={`text-[10px] uppercase tracking-wide ${weekTotals.available < 0 ? 'text-red-600' : 'text-emerald-700'}`}>
              {language === 'es' ? 'Disponible' : 'Available'}
            </div>
            <div className={`text-lg font-bold ${weekTotals.available < 0 ? 'text-red-700' : 'text-emerald-700'}`}>
              {formatHours(weekTotals.available)}h
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 p-4 sm:p-0">
            <div className="brand-panel w-full max-w-[95vw] sm:max-w-md max-h-screen overflow-auto">
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                {/* Hire / Termination Date Fields */}
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-sm font-medium text-[#4f3a70] mb-2">
                      {language === 'es' ? 'Fecha de alta' : 'Hire date'}
                    </label>
                    <input
                      type="date"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-[#4f3a70] mb-2">
                      {language === 'es' ? 'Fecha de baja' : 'Termination date'}
                    </label>
                    <input
                      type="date"
      ...
      <div className="overflow-y-auto flex-1 min-h-0 p-6">
        ...
        {DEPARTMENTS.map((dept) => {
          const allDeptEmployees = employees.filter(
            (e) => e.department === dept && !isPlaceholderEmployee(e) && !isCompanyPlaceholder(e)
          );
          const deptEmployees = allDeptEmployees.filter(matchesAvailabilityFilters);
          if (allDeptEmployees.length === 0) return null;
          ...
          return (
            <div key={dept} className={`mb-8 border rounded-lg p-4 ${getDepartmentColor(dept)}`}>
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                ...
                <button
                  type="button"
                  onClick={() => void handleExportDepartmentResources(dept, allDeptEmployees)}
              ...
              <div className="space-y-2">
                ...
                {deptEmployees.map((emp) => {
                  ...
                  const weekInfo = getWeekBreakdown(emp, selectedWeek);
                  const isOverAllocated = weekInfo.availableHours < 0;
                  ...
                  return (
                    <div key={emp.id} className="bg-white rounded-lg border border-gray-300 overflow-hidden">
                          ...
                          <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 items-center">
                            <div>
                              ...
                              {(emp.hireDate || emp.terminationDate) && (
                                <div className="text-[10px] text-gray-400 mt-0.5">
                                  {emp.hireDate && (
                                    <span>{language === 'es' ? 'Alta' : 'Hired'}: {emp.hireDate}</span>
                                  )}
                                  {emp.hireDate && emp.terminationDate && <span> · </span>}
                                  {emp.terminationDate && (
                                    <span className="text-red-500 font-medium">
                                      {language === 'es' ? 'Baja' : 'Terminated'}: {emp.terminationDate}
                                    </span>
                                  )}
                                </div>
                              )}
                            ...
                            <div className="text-center">
                              <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                                isEffectivelyActive(emp) ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                              }`}>
                                {isEffectivelyActive(emp) ? t.active : t.inactive}
                              </span>
                            </div>
                            <div className="text-center">
                              {!weekInfo.activeThisWeek ? (
                                <span className="text-xs text-gray-400" title={language === 'es' ? 'Fuera del rango de alta/baja esta semana' : 'Outside the hire/termination range this week'}>
                                  N/A
                                </span>
                              ) : (
                                <div className="flex items-center justify-center gap-2">
                                  <span className="text-xs bg-slate-100 text-slate-700 px-2 py-0.5 rounded-full font-medium">
                                    {formatHours(weekInfo.assignedHours)}h {language === 'es' ? 'asig.' : 'assigned'}
                                  </span>
                                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                                    isOverAllocated ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'
                                  }`}>
                                    {formatHours(weekInfo.availableHours)}h {language === 'es' ? 'disp.' : 'avail.'}
                                  </span>
                                </div>
                              )}
                            </div>
                      ...
                      {/* This week's project/department breakdown - merged in from the former standalone Availability page */}
                      {weekInfo.breakdown.length > 0 && (
                        <div className="px-3 pb-2 flex flex-wrap gap-1">
                          {weekInfo.breakdown.map((entry, idx) => (
                            <span
                              key={idx}
                              ...
                              title={entry.projectId ? projectNameById.get(entry.projectId) : undefined}
                            >
                              {entry.isSupport && '🤝 '}
                              {entry.department}
