...
export const parseISODate = (dateStr: string): Date => {
  ...
  if (Number.isFinite(year) && Number.isFinite(month) && Number.isFinite(day)) {
    // Construct at LOCAL midnight, not Date.UTC. Every other helper in this file (and every
    // caller across the app) reads this value back with local getters/setters (.getDate(),
    // .getMonth(), .getDay(), .setDate() via formatToISO/getWeekStart/getISOWeekInfo/etc).
    // Building it in UTC while everything downstream reads it locally silently shifts the
    // calendar day backward by one in any UTC-negative timezone (all of the Americas,
    // including America/Mexico_City, where this app is used) — e.g. it turned a Monday into
    // "the previous Sunday" for week-number/day-of-month purposes. Local construction matches
    // what every consumer already assumes, so the round trip (parse -> arithmetic ->
    // formatToISO) stays correct without touching every call site.
